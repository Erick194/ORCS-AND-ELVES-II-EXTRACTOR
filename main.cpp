...
static int Create_Walls(int wallindex,int paloffset,int id)
{
    ...
    sprintf(string,"WTEXELS/WALL%03d",id/4);
...
int Create_Sprites(int bitshapeindex,int paloffset,int id)
{
    ...
    sprintf(string,"STEXELS/STEX%03d",id/4);
    ZipAdd(hz, string, pngout, pngsize);
...
void ShowInfo()
{
    ...
    setcolor(0x07);printf("#############\n"); 
    printf("     #    ORCS AND ELVES 2 EXTRACTOR   #\n");
    printf("     # CREADO POR ERICK VASQUEZ GARCIA #\n");
    printf("     #     ES PARA ORCS AND ELVES 2    #\n");
    printf("     #         VERSION 4.1.30          #\n");
    printf("     #          MODO DE USO:           #\n");
...
void ExtraerDatos()
{
    hz = OpenZip(("orcs_and_elves_2_v_4.1.30.jar"),0);
    if(hz == NULL){error("No encuentra orcs_and_elves_2_v_4.1.30.jar");}
...
int main(int argc, char *argv[])
{
    ...
    hz = CreateZip("OrcsAndElvesII.pk3",0);
    
    int count1 = ReadWord(f1);
    int count2 = ReadWord(f1);
    int count3 = ReadWord(f1);
    int count4 = ReadWord(f1);
    int i;
    int bitshapeindex;
    int paloffset;
    int wallindex;
    ...
    if (count1 > 41)
    {
     error("Error count1 supera el maximo de 41");
    }
    ...
    for(i = 0; i < count1; i+=4)
    {
        ...
        wallindex = ReadWord(f1); //printf("wallindex %d\n", wallindex);
        paloffset = 16 * ReadWord(f1); //printf("paloffset %d\n", paloffset);
    ...
    if (count2 > 475)
    {
     error("Error count2 supera el maximo de 475");
    }
    ...
    for(int i = 0; i < count2; i+=4)
    {
        ...
        bitshapeindex = ReadWord(f1);//printf("bitshapesoffset %d\n",bitshapesoffset);
        paloffset = 16 * ReadWord(f1);//printf("paloffset %d\n", paloffset);
